            ...
            <Fragment key={i}>
              <SelectComponent
                ...
                data={
                  i > indexTillShow
                    ? []
                    : uniqueValueFromArray(
                        products.map((product) => product[item.value])
                      )
                }
                titleValue={item}
...
function SelectComponent({
  ...
  return (
    <label className='form-control w-full max-w-xs'>
      ...
      <select
        defaultValue={selectedValue ?? 'Pick one'}
        onChange={(e) => onClickOption(e.target.value)}
        className='select select-bordered select-primary capitalize'
      >
